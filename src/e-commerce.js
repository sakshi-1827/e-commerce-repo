...
const Ecommerce = () => {
    ...
    useEffect(() => {
        const storedCart = JSON.parse(localStorage.getItem('CartDetails')) || [];
        setMyArray(storedCart);
    }, []);

    const addItem = (item) => {
        if (!myArray.includes(item)) {
            setMyArray([...myArray, item]);
            localStorage.setItem('CartDetails', JSON.stringify([...myArray, item]));
        } else {
    ...
    return (
        <>
            ...
            <div className='div2'>
                ...
                {
                    data.filter(data => data.Item === "BigSale").map((filterdata) => (
                        <div className='banner' >
                            <div className='bigsale'>BIG SALE 50%</div>
                            <span className='bd' >{filterdata?.productname}</span>
                            <br></br>
                            <span className='bd' >New smart watch</span><br></br>
                            <img className='img1' src={filterdata?.image} height={115} width={115} /><br></br><br></br>
                            <div className='bigsale'>UGX {filterdata?.Price}</div><br></br>
                            <button className='addtocart' onClick={() => addItem(filterdata.id)}>Add to Cart</button>
                        </div>
                    ))
                }
            </div>
